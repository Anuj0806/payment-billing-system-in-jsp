...
<%
    ...
    String branch = request.getParameter("mobile");    
    String sap = request.getParameter("feesub");
    String doj = request.getParameter("paid");  
    String sal = request.getParameter("trainer");

    try {
        Class.forName("com.mysql.cj.jdbc.Driver");
        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/payment", "root", "Anuj0806$");

        PreparedStatement ps = con.prepareStatement("select doj from sal where id like '%" + id + "%'");
//ps.setString(1,n);
...
//            out.print(day_gap);
  if(day_gap<30){
   out.println("<script type=\"text/javascript\">");
        ...
        out.println("location='manualpay.jsp';");
        out.println("</script>");
    }
    else{
    try {
                        ...
                        Class.forName("com.mysql.cj.jdbc.Driver");
                        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/payment", "root", "Anuj0806$");
                        String sql="update sal set name='"+name+"',branch='"+branch+"',specialization='"+sap+"',doj='"+current_date+"',salary='"+sal+"' where id= '"+id+"'";
